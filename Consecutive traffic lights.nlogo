globals [
  ...
  traffic-lights ; a list of the x coordinates of traffic lights
]
...
[
  ...
  intersection-row          ;; the row of the intersection counting from the upper left corner of the
...
to draw-road-lines
  ...
  ; reference lines
  draw-vert-line -126 red 0.5
  draw-vert-line 126 red 0.5
end

to make-traffic-lights
  let first-light ( 1 - number-of-lights ) * light-spacing / 2
  set traffic-lights n-values number-of-lights [ n -> floor ( first-light + n * light-spacing ) ]
  foreach traffic-lights
  [
    ;x -> draw-vert-line x blue 0
    x -> ask patch x (max-pycor - 1) [ set plabel "Stop!" ]

  ]
  ask patches with [ member? pxcor traffic-lights and abs pycor < number-of-lanes ] [
    sprout-lights 1 [
      set color red
      set delay ( position pxcor traffic-lights * change-delay )
...
to draw-vert-line [ x line-color gap]
  create-turtles 1 [
    setxy x max-pycor
    hide-turtle
    set color line-color
...
to change-lights
  ask lights
  [
    let complete-cycle green-duration + yellow-duration + red-duration + yellow-duration

    if ( ticks - delay ) mod complete-cycle = 0
    [
      set color green
      ask patch xcor (max-pycor - 1) [ set plabel "Go!" ]

    ]

    if ( ticks - delay - green-duration ) mod complete-cycle = 0
    [
      set color yellow
      ask patch xcor (max-pycor - 1) [ set plabel "" ]
    ]

    if ( ticks - delay - green-duration - yellow-duration ) mod complete-cycle = 0
    [
      set color red
      ask patch xcor (max-pycor - 1) [ set plabel "Stop!" ]
    ]

    if ( ticks - delay - green-duration - yellow-duration - red-duration ) mod complete-cycle = 0
    [
      set color yellow
      ask patch xcor (max-pycor - 1) [ set plabel "" ]
    ]
...
to move-forward ; turtle procedure
  ...
  let red-lights lights in-cone (1 + speed) 180 with [ color = red ]

  let blocking-car min-one-of blocking-cars [ distance myself ]
  let red-light min-one-of red-lights [ distance myself ]
...
1665
740
Proportion of time spent waiting
NIL
NIL
0.0
10.0
...
"" ""
PENS
"average" 1.0 0 -10899396 true "" "plot ( mean [ wait-time ] of cars ) / (ticks)"
"max" 1.0 0 -11221820 true "" "plot ( max [ wait-time ] of cars ) / (ticks)"
"min" 1.0 0 -13345367 true "" "plot ( min [ wait-time ] of cars ) / (ticks)"
"selected car" 1.0 0 -2674135 true "" "plot ( [ wait-time ] of selected-car ) / (ticks)"
...
PENS
"default" 0.05 1 -16777216 true "" "histogram ([ wait-time / ticks ] of cars)"
...
450
715
Model is based on the following Sample Models:\n- Traffic 2 Lanes\n- Traffic Grid
15
0.0
1
